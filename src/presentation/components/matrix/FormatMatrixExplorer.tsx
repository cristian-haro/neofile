...
import { FormatRegistry, FormatCategory, FormatDefinition } from '../../../core/domain/entities/Format';
    ...
    <div className="w-full space-y-6 animate-fade-in mb-16">
      ...
      <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 sm:p-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-4 shrink-0 bg-slate-950 border border-slate-800 px-4 py-2 rounded-xl">
            <div>
              <span className="text-[10px] uppercase font-mono text-slate-500 block">Total Formats</span>
              <span className="text-lg font-bold font-mono text-blue-400">{allFormats.length}</span>
            </div>
            <div className="h-8 w-px bg-slate-800" />
            <div>
              <span className="text-[10px] uppercase font-mono text-slate-500 block">Conversion Paths</span>
              <span className="text-lg font-bold font-mono text-emerald-400">{totalPairs}+</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-blue-500 font-mono transition-colors"
            />
          </div>
                ...
                <button
                  ...
                  className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                    isSelected
                      ...
                      : 'bg-slate-950 border border-slate-800/80 text-slate-400 hover:text-slate-200 hover:bg-slate-800'
                  }`}
                >
          ...
          <table className="w-full text-left text-xs text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {filteredFormats.map(format => {
                ...
                return (
                  <tr key={format.extension} className="hover:bg-slate-900/50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap font-mono">
                      <span className="font-bold text-slate-100 text-sm">.{format.extension.toUpperCase()}</span>
                      {format.isLossless && (
                        <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-mono bg-emerald-950 text-emerald-300 border border-emerald-800">
                          {t.matrix.lossless}
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-medium bg-slate-950 border border-slate-800 text-slate-300 capitalize">
                        {format.category}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap items-center gap-1.5 max-w-lg">
                        {targets.slice(0, 7).map(tgt => (
                          <span
                            key={tgt.extension}
                            className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono bg-slate-950 border border-slate-800 text-blue-300"
                          >
