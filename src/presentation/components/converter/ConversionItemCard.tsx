...
export const ConversionItemCard: React.FC<ConversionItemCardProps> = ({ job }) => {
  const { t } = useI18n();
  const { updateJobTarget, updateJobOptions, convertSingleJob, removeJob, getCompatibleTargets } = useConverter();
  ...
  const getCategoryIcon = (category?: string) => {
    switch (category) {
      ...
      case 'ebook': return <BookOpen className="w-5 h-5 text-emerald-400" />;
      case 'compressed': return <Archive className="w-5 h-5 text-cyan-400" />;
      case 'cad': return <Compass className="w-5 h-5 text-orange-400" />;
  ...
  const renderStatusBadge = () => {
    switch (job.status) {
      case 'queued':
        return (
          <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-md text-[11px] font-medium bg-slate-800 text-slate-300 border border-slate-700">
      ...
      case 'converting':
        return (
          <span className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md text-[11px] font-medium bg-blue-950 text-blue-300 border border-blue-800">
      ...
      case 'completed':
        return (
          <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-md text-[11px] font-medium bg-emerald-950 text-emerald-300 border border-emerald-800">
      ...
      case 'failed':
        return (
          <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-md text-[11px] font-medium bg-rose-950 text-rose-300 border border-rose-800" title={job.error}>
  ...
  return (
    <>
      <div className="bg-slate-900/70 border border-slate-800/80 rounded-xl p-4 sm:p-5 transition-all duration-150 hover:border-slate-700/80 shadow-sm">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          
          {/* File Info */}
          <div className="flex items-center space-x-3.5 min-w-0 max-w-full md:max-w-[40%]">
            ...
            <div className="min-w-0">
              <h4 className="text-sm font-semibold text-slate-100 truncate" title={job.sourceFile.name}>
                {job.sourceFile.name}
              </h4>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            <div className="flex items-center space-x-2">
              <span className="text-xs text-slate-400">{t.item.convertTo}:</span>
              <select
              ...
              >
                {compatibleTargets.length > 0 ? (
                  ...
                  <option value={job.targetFormat.extension}>
          ...
          <div className="flex items-center space-x-2 self-end md:self-center shrink-0">
            ...
            {job.status === 'queued' && (
              <button
                ...
                className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-medium shadow-sm transition-colors"
              >
            ...
            {job.status === 'failed' && (
              <button
                onClick={() => convertSingleJob(job.id)}
                className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-500 text-white text-xs font-medium shadow-sm transition-colors"
              >
                <Play className="w-3.5 h-3.5" />
            ...
            <button
              ...
              disabled={job.status === 'converting'}
              className="p-1.5 rounded-lg text-slate-500 hover:text-rose-400 hover:bg-rose-950/30 border border-transparent hover:border-rose-900/50 transition-colors disabled:opacity-50"
              title={t.item.remove}
            >
        ...
        {job.status === 'failed' && job.error && (
          <div className="mt-2 text-[11px] font-mono text-rose-400 bg-rose-950/40 border border-rose-900/40 rounded-lg p-2">
            Error: {job.error}
          </div>
        )}
      </div>
